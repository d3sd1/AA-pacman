    ...
    def run(self):
        ...
        while not self.gameOver:
            ...
            if hasattr(agent, 'printLineData'):
                ...
                # training_keyboard.arff -> 500 NEW instances, 5 maps (20Hunt, bigHunt, capsuleClassic, classic, contestClassic), random ghosts, control by keyboard
                # test_samemaps_keyboard.arff -> 200 NEW instances, 5 maps (20Hunt, bigHunt, capsuleClassic, classic, contestClassic), random ghosts, control by keyboard
                # test_othermaps_keyboard.arff -> 200 NEW instances, 5 maps (mediumClassic, Minimapa, minimaxClassic, newMap, oneHunt), random ghosts, control by keyboard

                # training_tutorial1.arff -> 500 NEW instances, 5 maps (20Hunt, bigHunt, capsuleClassic, classic, contestClassic), random ghosts, random control
                ...
                fPath = "test_othermaps_keyboard.arff"
                # agentType = type(agent)
                # if agentType == 'ghostAgents.RandomGhost':
                #     fPath = "all_data_pacman.arff"
                # elif agentType == 'bustersAgents.BustersKeyboardAgent':
                #     fPath ="all_data_pacman.arff"
                # else:
                #     fPath = "all_data_pacman.arff"

                # If file does not exists initialize ARFF!
                if os.path.exists(fPath) == False:
                    f = open(fPath, "a")
                    f.write("@RELATION pacman_data_andrei\n")
                    f.write("@ATTRIBUTE coords_x NUMERIC\n")
                    f.write("@ATTRIBUTE coords_y NUMERIC\n")
                    f.write("@ATTRIBUTE nearest_dot_distance NUMERIC\n")
                    f.write("@ATTRIBUTE phantom_count NUMERIC\n")
                    f.write("@ATTRIBUTE current_score NUMERIC\n")
                    f.write("@ATTRIBUTE capsules_count NUMERIC\n")
                    f.write("@ATTRIBUTE pacman_current_direction {East,North,South,West,Stop}\n")
                    f.write("@ATTRIBUTE pacman_next_direction {East,North,South,West,Stop}\n")
                    f.write("@ATTRIBUTE next_score NUMERIC\n")
                    f.write("@DATA\n")
                    f.close()
